...
app.post("/apostar", function (req, res) {
    ...
    // Converte as strings de diaAposta e diaJogo em objetos Date
    const diaAposta = new Date(obj.diaAposta);
    const diaJogo = new Date(obj.diaJogo);

    if (diaAposta > diaJogo) {
        console.log(5);
        res.status(400).json({ "msg": "Apostas encerradas para o evento" });
    } else {
        const token = req.cookies.token;
        const decoded = jwt.verify(token, process.env.SECRET);
        const id_user = decoded.id;
                    ...
                    if (error) {
                        ...
                        db.atualizarSaldo({ id_user: id_user, saldo: novoSaldo }, function (error) {
                            if (error) {
                                ...
                                res.status(200).json({ "msg": "Apostas realizada com sucesso" });
                            }
                        });
                    }
                });
